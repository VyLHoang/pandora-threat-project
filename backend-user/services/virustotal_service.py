...
class VirusTotalService:
    ...
    def __init__(self):
        ...
        if not self.api_key or self.api_key == "":
            ...
            print("[INFO] Scanner will return mock data for development")
            self.api_key = None
    ...
    def scan_ip(self, ip_address: str, use_cache: bool = True) -> Optional[Dict]:
        ...
        if not self.api_key:
            print(f"[MOCK] Returning mock data for IP: {ip_address}")
    ...
    def scan_file_hash(self, file_hash: str, use_cache: bool = True) -> Optional[Dict]:
        ...
        # If no API key, return mock data
        if not self.api_key:
            print(f"[MOCK] Returning mock data for hash: {file_hash}")
            return self._get_mock_hash_data(file_hash)
    ...
    def _get_mock_ip_data(self, ip_address: str) -> Dict:
        ...
        return {
            ...
            "reputation": 0,
            "engine_results": [
                {"engine": "MockEngine", "category": "harmless", "result": "clean"},
            ],
            "mock": True,
            "raw_response": {"mock": True}
        }
    
    def _get_mock_hash_data(self, file_hash: str) -> Dict:
        """Return mock data for file hash without API key"""
        return {
            "target": file_hash,
            "is_malicious": False,
            "detection_count": 0,
